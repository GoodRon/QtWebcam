...
QImage makeARGB32Image(const unsigned char* data, unsigned len,
	...
	return simpleQimage(data, width, height, QImage::Format_ARGB32).rgbSwapped();
}

QImage makeRGB32Image(const unsigned char* data, unsigned len,
                      unsigned width, unsigned height) {
	return simpleQimage(data, width, height, QImage::Format_RGB32).rgbSwapped();
}

QImage makeRGB24Image(const unsigned char* data, unsigned len,
                      unsigned width, unsigned height) {
	return simpleQimage(data, width, height, QImage::Format_RGB888).rgbSwapped();
}

QImage makeRGB555Image(const unsigned char* data, unsigned len,
                       unsigned width, unsigned height) {
	return simpleQimage(data, width, height, QImage::Format_RGB555).rgbSwapped();
}
